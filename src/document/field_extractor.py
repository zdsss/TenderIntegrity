...
# 专业角色关联人名（Q4：扩展团队成员提取范围）
_TEAM_ROLE_PATTERNS = [
    re.compile(r"(?:项目经理|项目负责人)[：:\s]\s*([\u4e00-\u9fa5]{2,4})"),
    re.compile(r"(?:系统架构师|技术架构师|首席架构师)[：:\s]\s*([\u4e00-\u9fa5]{2,4})"),
    re.compile(r"(?:实施顾问|交付经理|售前顾问|实施经理)[：:\s]\s*([\u4e00-\u9fa5]{2,4})"),
    re.compile(r"(?:测试工程师|测试经理|质量工程师)[：:\s]\s*([\u4e00-\u9fa5]{2,4})"),
    re.compile(r"(?:运维工程师|运维经理|安全工程师)[：:\s]\s*([\u4e00-\u9fa5]{2,4})"),
    re.compile(r"(?:数据库工程师|数据架构师)[：:\s]\s*([\u4e00-\u9fa5]{2,4})"),
]
...
class KeyFieldExtractor:
    ...
    def extract(self, chunks: list[ParagraphChunk]) -> KeyFields:
        ...
        for chunk in chunks:
            ...
            # 扩展团队成员提取（Q4）
            for pattern in _TEAM_ROLE_PATTERNS:
                for m in pattern.finditer(text):
                    team_members.add(m.group(1))
        ...
        logger.debug(
            ...
            f"团队成员={len(result.team_members)}"
        )
        return result
